...
!   sources: -
!   includes: ../interp/gyre_interp.inc ../extern/core/core.inc ../extern/core/core_parallel.inc
!   uses: ISO_FORTRAN_ENV gyre_calc core_kinds core_hgroup core_order core_linalg core_parallel
...
contains
  ...
  if(SIZE(f)/= SIZE(x)) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(f)==SIZE(x) failed at line 20 <gyre_c_interp:c_interp_t_>'
    stop
  endif
  ...
  if(SIZE(df_dx)/= SIZE(x)) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'SIZE(x) :', SIZE(x)
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(df_dx)==SIZE(x) failed at line 20 <gyre_c_interp:c_interp_t_>'
    stop
  endif
  ...
  if(SIZE(f)/= SIZE(x)) then
    write(UNIT=ERROR_UNIT, FMT=*) 'SIZE(f) :', SIZE(f)
    write(UNIT=ERROR_UNIT, FMT=*) 'SIZE(x) :', SIZE(x)
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(f)==SIZE(x) failed at line 20 <gyre_c_interp:c_interp_t_eval_derivs_>'
    stop
  endif
