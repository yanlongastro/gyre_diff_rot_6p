...
!   uses: core_kinds core_order ISO_FORTRAN_ENV core_linalg
...
contains
  ...
  subroutine eigen_decompose_r_ (A, lambda, V_l, V_r, sort)
    ...
    if(.NOT. (info == 0)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''info == 0'' failed at line 160 <gyre_linalg:eigen_decompose_r_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Non-zero return from XGEEV'
      stop
    endif
...
!          $ASSERT(ABS(norm) > n*EPSILON(0._WP),Near-defective matrix)
  ...
  contains
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'ABORT at line 230 <gyre_linalg:eigen_decompose_r_:reconstruct_eigenvector_>:'
    write(UNIT=ERROR_UNIT, FMT=*) 'Complex eigenvalue does not have conjugate pair'
  ...
  subroutine eigen_decompose_c_ (A, lambda, V_l, V_r, sort)
    ...
    if(.NOT. (info == 0)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''info == 0'' failed at line 294 <gyre_linalg:eigen_decompose_c_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Non-zero return from XGEEV'
      stop
    endif
...
!          $ASSERT(ABS(norm) > n*EPSILON(0._WP),Near-defective matrix)
  ...
  subroutine eigen_decompose_2_r_ (A, lambda, V_l, V_r)
  ...
  end subroutine eigen_decompose_2_r_

  subroutine eigen_decompose_2_c_ (A, lambda, V_l, V_r)
  ...
  end subroutine eigen_decompose_2_c_
  ...
  subroutine sing_decompose_r_ (A, sigma, U, V_H)
    ...
    if(.NOT. (info == 0)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''info == 0'' failed at line 450 <gyre_linalg:sing_decompose_r_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Non-zero return from XGESVD'
      stop
    endif
  ...
  subroutine sing_decompose_c_ (A, sigma, U, V_H)
    ...
    if(.NOT. (info == 0)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''info == 0'' failed at line 485 <gyre_linalg:sing_decompose_c_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Non-zero return from XGESVD'
      stop
    endif
  ...
  function linear_solve_vec_r_ (A, b) result (x)
  ...
  end function linear_solve_vec_r_

  function linear_solve_mat_r_ (A, B) result (X)

    real(WP), intent(in) :: A(:,:)
    real(WP), intent(in) :: B(:,:)
    real(WP)             :: X(SIZE(A, 2), SIZE(B, 2))

    real(WP)    :: A_(SIZE(A, 1),SIZE(A, 2))
    real(WP)    :: B_(SIZE(B, 1),SIZE(B, 2))
    integer      :: ipiv(SIZE(A, 1))
    integer      :: info
    real(WP)    :: Af(SIZE(A, 1),SIZE(A, 2))
    real(WP)     :: R(SIZE(A, 1))
    real(WP)     :: C(SIZE(A, 1))
    real(WP)     :: rcond
    ...
    if(.NOT. (info == 0)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''info == 0'' failed at line 572 <gyre_linalg:linear_solve_mat_r_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Non-zero return from XGESVX'
      stop
    endif
  ...
  end function linear_solve_mat_r_

  function linear_solve_vec_c_ (A, b) result (x)

    complex(WP), intent(in) :: A(:,:)
    complex(WP), intent(in) :: b(:)
    complex(WP)             :: x(SIZE(A, 2))
  ...
  end function linear_solve_vec_c_

  function linear_solve_mat_c_ (A, B) result (X)

    complex(WP), intent(in) :: A(:,:)
    complex(WP), intent(in) :: B(:,:)
    complex(WP)             :: X(SIZE(A, 2), SIZE(B, 2))

    complex(WP)    :: A_(SIZE(A, 1),SIZE(A, 2))
    complex(WP)    :: B_(SIZE(B, 1),SIZE(B, 2))
    integer      :: ipiv(SIZE(A, 1))
    integer      :: info
    complex(WP)    :: Af(SIZE(A, 1),SIZE(A, 2))
    real(WP)     :: R(SIZE(A, 1))
    real(WP)     :: C(SIZE(A, 1))
    real(WP)     :: rcond
    real(WP)     :: ferr(SIZE(B, 2))
    ...
    if(.NOT. (info == 0)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''info == 0'' failed at line 573 <gyre_linalg:linear_solve_mat_c_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Non-zero return from XGESVX'
      stop
    endif
  ...
  end function linear_solve_mat_c_

  !****

  function commutator_r_ (A, B) result (C)
  ...
  end function commutator_r_

  function commutator_c_ (A, B) result (C)
  ...
  end function commutator_c_

  !****

  subroutine measure_bandwidth_full_r_ (A, n_l, n_u)
    ...
    lower_loop : do k = -(n-1), -1
       ...
       do i = i_min, i_max
          j = i + k
          if(A(i,j) /= 0._WP) exit lower_loop
       end do
    ...
    upper_loop : do k = n-1, 1, -1
       ...
       do i = i_min, i_max
          j = i + k
          if(A(i,j) /= 0._WP) exit upper_loop
       end do
  ...
  end subroutine measure_bandwidth_full_r_

  subroutine measure_bandwidth_full_c_ (A, n_l, n_u)
    ...
    lower_loop : do k = -(n-1), -1
       ...
       do i = i_min, i_max
          j = i + k
          if(A(i,j) /= (0._WP,0._WP)) exit lower_loop
       end do
    ...
    upper_loop : do k = n-1, 1, -1
       ...
       do i = i_min, i_max
          j = i + k
          if(A(i,j) /= (0._WP,0._WP)) exit upper_loop
       end do
  ...
  end subroutine measure_bandwidth_full_c_

  !****

  pure function diagonal_r_ (A) result (D)

    real(WP), intent(in) :: A(:,:)
    real(WP)             :: d(MIN(SIZE(A, 1),SIZE(A, 2)))
  ...
  end function diagonal_r_

  pure function diagonal_c_ (A) result (D)

    complex(WP), intent(in) :: A(:,:)
    complex(WP)             :: d(MIN(SIZE(A, 1),SIZE(A, 2)))
  ...
  end function diagonal_c_

  !****

  pure function diagonal_matrix_r_ (x) result (D)

    real(WP), intent(in) :: x(:)
    real(WP)             :: D(SIZE(x), SIZE(x))
  ...
  end function diagonal_matrix_r_

  pure function diagonal_matrix_c_ (x) result (D)

    complex(WP), intent(in) :: x(:)
    complex(WP)             :: D(SIZE(x), SIZE(x))
  ...
  end function diagonal_matrix_c_
  ...
  function matrix_exp_r_ (A, t) result (exp_At)

    real(WP), intent(in) :: A(:,:)
    real(WP), intent(in)  :: t
    real(WP)             :: exp_At(SIZE(A, 1),SIZE(A, 2))
    ...
    call eigen_decompose_r_(A_, lambda, V_l=V_l, V_r=V_r)
  ...
  end function matrix_exp_r_

  function matrix_exp_c_ (A, t) result (exp_At)

    complex(WP), intent(in) :: A(:,:)
    real(WP), intent(in)  :: t
    complex(WP)             :: exp_At(SIZE(A, 1),SIZE(A, 2))
    ...
    call eigen_decompose_c_(A_, lambda, V_l=V_l, V_r=V_r)
  ...
  function outer_product_r_ (v, w) result (A)

    real(WP), intent(in) :: v(:)
    real(WP), intent(in) :: w(:)
    real(WP)             :: A(SIZE(v), SIZE(w))
  ...
  end function outer_product_r_

  function outer_product_c_ (v, w) result (A)

    complex(WP), intent(in) :: v(:)
    complex(WP), intent(in) :: w(:)
    complex(WP)             :: A(SIZE(v), SIZE(w))
  ...
  subroutine partition_r_ (A, mask_r, mask_c, A_part)
  ...
  end subroutine partition_r_

  subroutine partition_c_ (A, mask_r, mask_c, A_part)
