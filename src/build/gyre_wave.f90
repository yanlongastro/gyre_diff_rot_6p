...
!   uses: gyre_context ISO_FORTRAN_ENV gyre_constants gyre_state gyre_grid core_kinds gyre_point gyre_evol_model gyre_mode_par gyre_util gyre_osc_par gyre_model gyre_freq gyre_rot gyre_grid_util gyre_ext core_parallel
!   provides: gyre_wave
!end dependencies
!
!end fpx3_header
...
subroutine reallocate_1_ (array, shape_new, start)
  ...
  type(wave_t), allocatable, intent(inout) :: array(:)
  integer, intent(in)                     :: shape_new(:)
  integer, intent(in), optional           :: start(:)

  type(wave_t), allocatable :: array_new(:)
  integer                      :: i_a(1)
  integer                      :: i_b(1)
  integer                      :: i_c(1)
...
array_new(i_c(1):) = array(i_b(1):)
  ...
  function dW_eps_dx (this, k)
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'ABORT at line 912 <gyre_wave:dW_eps_dx>:'
    write(UNIT=ERROR_UNIT, FMT=*) 'Evaluating dW_dx_eps not supported for deps_scheme /= MODEL'
  ...
  function E_norm (this)
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'ABORT at line 1682 <gyre_wave:E_norm>:'
    write(UNIT=ERROR_UNIT, FMT=*) 'Invalid inertia_norm'
  ...
  stop 'Program aborted'
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'INFO at line 1686 <gyre_wave:E_norm>:'
    write(UNIT=ERROR_UNIT, FMT=*) 'Amplitude at x_ref is zero; not normalizing inertia'
