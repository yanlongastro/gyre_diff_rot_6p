...
!   uses: ISO_FORTRAN_ENV core_kinds gyre_ext gyre_num_par gyre_ext_func gyre_status
...
!****
  ...
  subroutine narrow_ (rf, rx_a, rx_b, rx_tol, nm_p, status, n_iter, n_iter_max, relative_tol, f_rx_a, f_rx_b)
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'ABORT at line 133 <gyre_r_root:narrow_>:'
    write(UNIT=ERROR_UNIT, FMT=*) 'Invalid r_root_solver'
...
!****
 ...
 & // ' at line 208 <gyre_r_root:narrow_brent_>:'
...
!****
  ...
  subroutine expand_ (rf, rx_a, rx_b, status, clamp_a, clamp_b, f_rx_a, f_rx_b)
    ...
    if(.NOT. (.NOT. (clamp_a_ .AND. clamp_b_))) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''.NOT. (clamp_a_ .AND. clamp_b_)'' failed at line 384 <gyre_r_root:expand_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Cannot clamp both points'
      stop
    endif

    if(.NOT. (rx_a /= rx_b)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''rx_a /= rx_b'' failed at line 386 <gyre_r_root:expand_>:'
