...
!   uses: gyre_status ISO_FORTRAN_ENV gyre_num_par gyre_bound gyre_diff core_kinds gyre_ext gyre_state gyre_sysmtx_factory gyre_sysmtx
!   provides: gyre_r_bvp
!end dependencies
!
!end fpx3_header
...
contains
  ...
  if(n_i+n_o/= n_e) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS n_i+n_o==n_e failed at line 20 <gyre_r_bvp:r_bvp_t_>'
    stop
  endif

    if(.NOT. (ALL(df%n_e == n_e))) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''ALL(df%n_e == n_e)'' failed at line 20 <gyre_r_bvp:r_bvp_t_>:'
  ...
  function soln_vec_hom (this) result (y)
    ...
    if(.NOT. (this%factored)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''this%factored'' failed at line 20 <gyre_r_bvp:soln_vec_hom>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Matrix has not been factorized'
      stop
    endif
  ...
  if(SIZE(w_i)/= this%n_i) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(w_i)==this%n_i failed at line 20 <gyre_r_bvp:soln_vec_inhom>'
    stop
  endif
  ...
  if(SIZE(w_o)/= this%n_o) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(w_o)==this%n_o failed at line 20 <gyre_r_bvp:soln_vec_inhom>'
    stop
  endif

    if(.NOT. (this%factored)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''this%factored'' failed at line 20 <gyre_r_bvp:soln_vec_inhom>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Matrix has not been factorized'
      stop
    endif
  ...
  if(SIZE(y,1)/= this%n_e) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(y,1)==this%n_e failed at line 20 <gyre_r_bvp:resd_vec>'
    stop
  endif
  ...
  if(SIZE(y,2)/= this%n_k) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(y,2)==this%n_k failed at line 20 <gyre_r_bvp:resd_vec>'
    stop
  endif

    if(.NOT. (.NOT. this%factored)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''.NOT. this%factored'' failed at line 20 <gyre_r_bvp:resd_vec>:'
