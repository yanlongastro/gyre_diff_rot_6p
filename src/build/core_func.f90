...
!****
  ...
  subroutine expand_bracket_ (this, x_a, x_b, f_x_a, f_x_b, clamp_a, clamp_b)
    ...
    if(.NOT. (.NOT. (clamp_a_ .AND. clamp_b_))) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''.NOT. (clamp_a_ .AND. clamp_b_)'' failed at line 106 <core_func:expand_bracket_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Cannot clamp both points'
      stop
    endif

    if(.NOT. (x_a /= x_b)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''x_a /= x_b'' failed at line 108 <core_func:expand_bracket_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Invalid initial bracket'
      stop
    endif
...
!****
  ...
  subroutine narrow_bracket_ (this, x_a, x_b, x_tol, f_x_a, f_x_b, n_iter, relative_tol)
    ...
    if(.NOT. ((f_a >= 0._WP .AND. f_b <= 0._WP) .OR. (f_a <= 0._WP .AND. f_b >= 0._WP))) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''(f_a >= 0._WP .AND. f_b <= 0._WP) .OR. (f_a <= 0._WP .AND. f_b >= 0._WP)'' failed' &
 & // ' at line 221 <core_func:narrow_bracket_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Root is not bracketed'
      stop
    endif
    ...
    if(.NOT. (i <= n_iter_)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''i <= n_iter_'' failed at line 325 <core_func:narrow_bracket_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Too many iterations'
      stop
    endif
...
!****
  ...
  function root_c_ (this, z_a, z_b, z_tol, f_z_a, f_z_b, n_iter, relative_tol) result (z)
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'ABORT at line 475 <core_func:root_c_>:'
  ...
  stop 'Program aborted'
    ...
    if(.NOT. (i <= n_iter_)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''i <= n_iter_'' failed at line 501 <core_func:root_c_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Too many iterations'
      stop
    endif
