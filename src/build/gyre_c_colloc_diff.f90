...
!   sources: -
!   includes: ../diff/gyre_colloc_diff.inc ../extern/core/core.inc
!   uses: ISO_FORTRAN_ENV gyre_linalg gyre_point core_linalg core_kinds gyre_ext gyre_state gyre_eqns gyre_diff
...
module gyre_c_colloc_diff
  ...
  type, extends (c_diff_t) :: c_colloc_diff_t
     private
  ...
  end type c_colloc_diff_t

  ! Interfaces

  interface c_colloc_diff_t
     module procedure c_colloc_diff_t_
  end interface c_colloc_diff_t

  ! Access specifiers

  private

  public :: c_colloc_diff_t

contains

  function c_colloc_diff_t_ (eq, pt_a, pt_b, scheme) result (df)

    class(c_eqns_t), intent(in) :: eq
    type(point_t), intent(in)      :: pt_a
    type(point_t), intent(in)      :: pt_b
    character(*), intent(in)       :: scheme
    type(c_colloc_diff_t)       :: df
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'ABORT at line 20 <gyre_c_colloc_diff:c_colloc_diff_t_>:'
  ...
  end function c_colloc_diff_t_

  !****

  subroutine build (this, st, E_l, E_r, scl)

    class(c_colloc_diff_t), intent(in) :: this
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'ABORT at line 20 <gyre_c_colloc_diff:build>:'
    write(UNIT=ERROR_UNIT, FMT=*) 'Invalid scheme'
  ...
  subroutine build_irk_ (this, st, a_ij, b_i, E_a, E_b)
    ...
    complex(WP), intent(out)                :: E_a(:,:)
    complex(WP), intent(out)                :: E_b(:,:)

    complex(WP) :: A(this%n_e,this%n_e,this%n_s)
    complex(WP) :: M(this%n_s*this%n_e,this%n_s*this%n_e)
    integer   :: i
    integer   :: i_a
    integer   :: i_b
    integer   :: j
