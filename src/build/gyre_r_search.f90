...
!   uses: core_kinds gyre_wave gyre_root gyre_state gyre_num_par gyre_rot_factory gyre_point gyre_freq gyre_rot core_parallel gyre_mode gyre_ad_bvp core_order gyre_scan_par gyre_status gyre_rad_bvp gyre_context gyre_constants gyre_grid gyre_osc_par gyre_discrim_func gyre_mode_par gyre_ext gyre_model gyre_util core_memory ISO_FORTRAN_ENV gyre_bvp
...
contains
  ...
  subroutine build_scan (cx, md_p, os_p, sc_p, omega)
    ...
    if(.NOT. (SIZE(sc_p) >=1)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''SIZE(sc_p) >=1'' failed at line 90 <gyre_r_search:build_scan>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Empty scan_par_t'
      stop
    endif
...
100    format(A)
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'ABORT at line 142 <gyre_r_search:build_scan>:'
  ...
  stop 'Program aborted'
            ...
            if (check_log_level('INFO')) then
               write(OUTPUT_UNIT, 110) 'added scan interval : ', omega_min, ' -> ', omega_max, ' (', n_freq, ' points, ', TRIM( &
 & grid_type), ')'
...
120            format(3X,A,E24.16,A,E24.16)
  ...
  subroutine check_scan (ml, gr, omega, md_p, os_p)
    ...
    if(.NOT. (ALL(omega_c > 0._WP) .OR. ALL(omega_c < 0._WP))) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''ALL(omega_c > 0._WP) .OR. ALL(omega_c < 0._WP)'' failed at line 218 <gyre_r_searc' &
 & // 'h:check_scan>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Critical layer encountered'
      stop
    endif
...
110    format(1X,A3,1X,A4,1X,A7,1X,A6,1X,A6,1X,A15,1X,A15,1X,A10,1X,A6)
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'ABORT at line 327 <gyre_r_search>:'
    write(UNIT=ERROR_UNIT, FMT=*) 'Invalid bp class'
