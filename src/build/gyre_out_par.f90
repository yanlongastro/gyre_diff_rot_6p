...
!   uses: gyre_constants ISO_FORTRAN_ENV core_kinds
!   provides: gyre_out_par
!end dependencies
!
!end fpx3_header
! Module   : gyre_out_par
...
contains
  ...
  subroutine read_out_par (unit, stage, ot_p)
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'ABORT at line 103 <gyre_out_par:read_out_par>:'
    write(UNIT=ERROR_UNIT, FMT=*) 'Invalid stage'
...
100 continue
    ...
    if(.NOT. (n_ot_p == 1)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''n_ot_p == 1'' failed at line 110 <gyre_out_par:read_out_par>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Input file should contain exactly one &ad_output and one &nad_output namelist'
      stop
    endif
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'ABORT at line 138 <gyre_out_par:read_out_par>:'
    write(UNIT=ERROR_UNIT, FMT=*) 'Invalid stage'
