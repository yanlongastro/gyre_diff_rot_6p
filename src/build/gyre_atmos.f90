...
!   uses: gyre_point core_kinds ISO_FORTRAN_ENV gyre_model
!   provides: gyre_atmos
!end dependencies
!
!end fpx3_header
...
contains
  ...
  function atmos_beta_r_ (V_g, As, U, c_1, omega, lambda) result (beta)
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'INFO at line 86 <gyre_atmos:atmos_beta_r_>:'
    write(UNIT=ERROR_UNIT, FMT=*) 'WARNING: Discarding imaginary part of atmospheric radial wavenumber'
  ...
  function atmos_beta_c_ (V_g, As, U, c_1, omega, lambda) result (beta)
    ...
    if(.NOT. (AIMAG(psi2) == 0._WP)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''AIMAG(psi2) == 0._WP'' failed at line 153 <gyre_atmos:atmos_beta_c_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Steady-state mode with complex propagation discriminant'
      stop
    endif
  ...
  subroutine eval_atmos_cutoff_freqs (V_g, As, U, c_1, lambda, omega_cutoff_lo, omega_cutoff_hi)
    ...
    if(.NOT. (omega_cutoff_hi >= omega_cutoff_lo)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''omega_cutoff_hi >= omega_cutoff_lo'' failed at line 217 <gyre_atmos:eval_atmos_cu' &
 & // 'toff_freqs>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Incorrect cutoff frequency ordering'
      stop
    endif
