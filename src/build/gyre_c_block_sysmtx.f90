...
!   uses: gyre_linalg core_kinds gyre_sysmtx gyre_ext core_parallel core_linalg ISO_FORTRAN_ENV
!   provides: gyre_c_block_sysmtx
!end dependencies
!
!end fpx3_header
...
module gyre_c_block_sysmtx
  ...
  type, extends (c_sysmtx_t) :: c_block_sysmtx_t
     private
     ...
     complex(WP), allocatable        :: scl_i(:)   ! Inner boundary scales
     complex(WP), allocatable        :: scl_o(:)   ! Outer boundary scales
     type(c_ext_t), allocatable :: scl(:)     ! Block scales
   contains
     private
  ...
  end type c_block_sysmtx_t

  ! Interfaces

  interface c_block_sysmtx_t
     module procedure c_block_sysmtx_t_
  end interface c_block_sysmtx_t

  ! Access specifiers

  private

  public :: c_block_sysmtx_t

  ! Procedures

contains

  function c_block_sysmtx_t_ (n, n_e, n_i, n_o) result (sm)

    integer, intent(in)       :: n
    integer, intent(in)       :: n_e
    integer, intent(in)       :: n_i
    integer, intent(in)       :: n_o
    type(c_block_sysmtx_t) :: sm

  if(n_i+n_o/= n_e) then
    write(UNIT=ERROR_UNIT, FMT=*) 'n_i+n_o :', n_i+n_o
    write(UNIT=ERROR_UNIT, FMT=*) 'n_e :', n_e
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS n_i+n_o==n_e failed at line 20 <gyre_c_block_sysmtx:c_block_sysmtx_t_>'
    stop
  endif
  ...
  end function c_block_sysmtx_t_

  !****

  subroutine set_B_i (this, B, scl)

    class(c_block_sysmtx_t), intent(inout) :: this
  ...
  if(SIZE(B, 1)/= this%n_i) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(B, 1)==this%n_i failed at line 20 <gyre_c_block_sysmtx:set_B_i>'
    stop
  endif
  ...
  if(SIZE(B, 2)/= this%n_e) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(B, 2)==this%n_e failed at line 20 <gyre_c_block_sysmtx:set_B_i>'
    stop
  endif
  ...
  if(SIZE(scl)/= this%n_i) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_i :', this%n_i
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(scl)==this%n_i failed at line 20 <gyre_c_block_sysmtx:set_B_i>'
    stop
  endif
  ...
  if(SIZE(B, 1)/= this%n_o) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(B, 1)==this%n_o failed at line 20 <gyre_c_block_sysmtx:set_B_o>'
    stop
  endif
  ...
  if(SIZE(B, 2)/= this%n_e) then
    write(UNIT=ERROR_UNIT, FMT=*) 'SIZE(B, 2) :', SIZE(B, 2)
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_e :', this%n_e
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(B, 2)==this%n_e failed at line 20 <gyre_c_block_sysmtx:set_B_o>'
    stop
  endif
  ...
  if(SIZE(scl)/= this%n_o) then
    write(UNIT=ERROR_UNIT, FMT=*) 'SIZE(scl) :', SIZE(scl)
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_o :', this%n_o
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(scl)==this%n_o failed at line 20 <gyre_c_block_sysmtx:set_B_o>'
    stop
  endif
  ...
  if(SIZE(E_l, 1)/= this%n_e) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_e :', this%n_e
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(E_l, 1)==this%n_e failed at line 20 <gyre_c_block_sysmtx:set_E>'
    stop
  endif
  ...
  if(SIZE(E_l, 2)/= this%n_e) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_e :', this%n_e
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(E_l, 2)==this%n_e failed at line 20 <gyre_c_block_sysmtx:set_E>'
    stop
  endif
  ...
  if(SIZE(E_r, 1)/= this%n_e) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_e :', this%n_e
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(E_r, 1)==this%n_e failed at line 20 <gyre_c_block_sysmtx:set_E>'
    stop
  endif
  ...
  if(SIZE(E_r, 2)/= this%n_e) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_e :', this%n_e
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(E_r, 2)==this%n_e failed at line 20 <gyre_c_block_sysmtx:set_E>'
    stop
  endif

    if(.NOT. (k >= 1)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''k >= 1'' failed at line 20 <gyre_c_block_sysmtx:set_E>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Invalid block index'
      stop
    endif

    if(.NOT. (k <= this%n)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''k <= this%n'' failed at line 20 <gyre_c_block_sysmtx:set_E>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Invalid block index'
      stop
    endif
  ...
  subroutine factor (this)
    ...
    complex(WP), parameter :: ONE = 1._WP
    ...
    if(.NOT. (info >= 0)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''info >= 0'' failed at line 20 <gyre_c_block_sysmtx:factor>:'
      ...
      stop
            ...
            call ZGEMM('N', 'N', n_e, n_e, n_e, -ONE, &
                          M_U(n_e+1,1), 2*n_e, M_E(1,1), 2*n_e, ONE, &
                          M_E(n_e+1,1), 2*n_e)

            call ZTRSM('L', 'U', 'N', 'N', n_e, n_e, &
                          ONE, M_U(1,1), 2*n_e, M_E(1,1), 2*n_e)
  ...
  function det (this)
    ...
    class(c_block_sysmtx_t), intent(in) :: this
    type(c_ext_t)                       :: det
    ...
    if(.NOT. (info >= 0)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''info >= 0'' failed at line 20 <gyre_c_block_sysmtx:det>:'
      write(UNIT=ERROR_UNIT, FMT=*) ' Negative return from XGETRF'
      stop
    endif
      ...
      det = product([c_ext_t(diagonal(M)),c_ext_t(this%scl_i),this%scl,c_ext_t(this%scl_o)])
  ...
  function soln_vec_hom (this) result (v)
    ...
    complex(WP), parameter :: ZERO = 0._WP
    complex(WP), parameter :: ONE = 1._WP

    complex(WP) :: M(2*this%n_e,2*this%n_e)
    real(WP)  :: sigma(2*this%n_e)
    complex(WP) :: U(2*this%n_e,2*this%n_e)
    complex(WP) :: V_H(2*this%n_e,2*this%n_e)
    integer   :: ipiv(2*this%n_e)
    integer   :: info
    integer   :: i_s
    real(WP)  :: M_s
    integer   :: i
    complex(WP) :: v_bound(2*this%n_e)
    ...
    if(.NOT. (info >= 0)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''info >= 0'' failed at line 20 <gyre_c_block_sysmtx:soln_vec_hom>:'
      write(UNIT=ERROR_UNIT, FMT=*) ' Negative return from XGETRF'
      stop
    endif
  ...
  function soln_vec_inhom (this, w_i, w_o) result (v)
    ...
    integer   :: ipiv(2*this%n_e)
    integer   :: info
    complex(WP) :: B(2*this%n_e, 1)
    complex(WP) :: v_bound(2*this%n_e)

  if(SIZE(w_i)/= this%n_i) then
    write(UNIT=ERROR_UNIT, FMT=*) 'SIZE(w_i) :', SIZE(w_i)
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_i :', this%n_i
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(w_i)==this%n_i failed at line 20 <gyre_c_block_sysmtx:soln_vec_inhom>'
    stop
  endif
  ...
  if(SIZE(w_o)/= this%n_o) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_o :', this%n_o
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(w_o)==this%n_o failed at line 20 <gyre_c_block_sysmtx:soln_vec_inhom>'
    stop
  endif
    ...
    if(.NOT. (info >= 0)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''info >= 0'' failed at line 20 <gyre_c_block_sysmtx:soln_vec_inhom>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Negative return from XGETRF'
      stop
    endif
  ...
  if(SIZE(v_bound)/= 2*this%n_e) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(v_bound)==2*this%n_e failed at line 20 <gyre_c_block_sysmtx:backsub_>'
    stop
  endif
  ...
  if(SIZE(v)/= this%n_e*(this%n+1)) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(v)==this%n_e*(this%n+1) failed at line 20 <gyre_c_block_sysmtx:backsub_>'
    stop
  endif
    ...
    associate (n => this%n, n_e => this%n_e, n_i => this%n_i)
      ...
      recon_loop : do
         ...
         backsub_loop : do k = 1, n-l, 2*l
            ...
            call ZGEMV('N', n_e, n_e, -ONE, this%E_l(:,:,k+l), n_e, v(i_a:i_a+n_e-1), 1, ZERO, v(i_b:i_b+n_e-1), 1)
            call ZGEMV('N', n_e, n_e, -ONE, this%E_r(:,:,k+l), n_e, v(i_c:i_c+n_e-1), 1, ONE, v(i_b:i_b+n_e-1), 1)
  ...
  if(SIZE(v)/= this%n_e*(this%n+1)) then
    write(UNIT=ERROR_UNIT, FMT=*) 'SIZE(v) :', SIZE(v)
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_e*(this%n+1) :', this%n_e*(this%n+1)
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(v)==this%n_e*(this%n+1) failed at line 20 <gyre_c_block_sysmtx:resd_vec>'
    stop
  endif
...
end module gyre_c_block_sysmtx
