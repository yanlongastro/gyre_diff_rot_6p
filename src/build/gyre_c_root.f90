...
!   uses: gyre_ext_func ISO_FORTRAN_ENV gyre_cimplex gyre_ext gyre_status gyre_num_par core_kinds
!   provides: gyre_c_root
!end dependencies
!
!end fpx3_header
! Module   : gyre_c_root
...
!****
  ...
  subroutine narrow_ (cf, np, cx_a, cx_b, cx_tol, status, n_iter, n_iter_max, relative_tol, f_cx_a, f_cx_b)
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'ABORT at line 138 <gyre_c_root:narrow_>:'
    write(UNIT=ERROR_UNIT, FMT=*) 'Invalid c_root_solver'
...
!****
  ...
  subroutine narrow_secant_ (cf, np, cx_a, cx_b, cx_tol, status, n_iter, n_iter_max, relative_tol, f_cx_a, f_cx_b)
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'ABORT at line 244 <gyre_c_root:narrow_secant_>:'
    write(UNIT=ERROR_UNIT, FMT=*) 'Singular correction in secant'
...
!****
  ...
  subroutine narrow_ridders_ (cf, np, cx_a, cx_b, cx_tol, status, n_iter, n_iter_max, relative_tol, f_cx_a, f_cx_b)
    ...
    if(.NOT. (cx_a /= cx_b)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''cx_a /= cx_b'' failed at line 319 <gyre_c_root:narrow_ridders_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Invalid initial pair'
      stop
    endif
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'ABORT at line 403 <gyre_c_root:narrow_ridders_>:'
    write(UNIT=ERROR_UNIT, FMT=*) 'Singular correction in secant'
...
!****
  ...
  subroutine narrow_simplex_ (cf, np, cx_a, cx_b, cx_tol, status, n_iter, n_iter_max, relative_tol, f_cx_a, f_cx_b)
    ...
    if(.NOT. (cx_a /= cx_b)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''cx_a /= cx_b'' failed at line 463 <gyre_c_root:narrow_simplex_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Invalid initial pair'
      stop
    endif
...
!****
  ...
  subroutine expand_ (cf, cx_a, cx_b, f_cx_tol, status, clamp_a, clamp_b, relative_tol, f_cx_a, f_cx_b)
    ...
    if(.NOT. (.NOT. (clamp_a_ .AND. clamp_b_))) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''.NOT. (clamp_a_ .AND. clamp_b_)'' failed at line 553 <gyre_c_root:expand_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Cannot clamp both points'
      stop
    endif
    ...
    if(.NOT. (cx_a /= cx_b)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''cx_a /= cx_b'' failed at line 561 <gyre_c_root:expand_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Invalid initial pair'
      stop
    endif
