...
contains
  ...
  function xlamch_r_sp_ (x, cmach)

    real(SP), intent(in)        :: x
    character(*), intent(in) :: cmach
    real(SP)                    :: xlamch_r_sp_

    real(SP), external :: SLAMCH

    xlamch_r_sp_ = SLAMCH(cmach)

  end function xlamch_r_sp_

  function xlamch_r_dp_ (x, cmach)

    real(DP), intent(in)        :: x
    character(*), intent(in) :: cmach
    real(DP)                    :: xlamch_r_dp_

    real(DP), external :: DLAMCH

    xlamch_r_dp_ = DLAMCH(cmach)

  end function xlamch_r_dp_
