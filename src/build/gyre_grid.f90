...
!   uses: gyre_point core_kinds ISO_FORTRAN_ENV core_parallel
!   provides: gyre_grid
!end dependencies
!
!end fpx3_header
...
contains
  ...
  function grid_t_resamp_ (gr_base, dn) result (gr)
    ...
    if(.NOT. (dn(j) == 0)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''dn(j) == 0'' failed at line 221 <gyre_grid:grid_t_resamp_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Attempt to add points at cell boundary'
      stop
    endif
