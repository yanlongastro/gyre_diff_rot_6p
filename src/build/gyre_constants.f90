...
!   uses: core_system core_kinds core_constants
...
100 continue
    ...
    if(.NOT. (n_cn == 1)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''n_cn == 1'' failed at line 112 <gyre_constants:read_constants>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Input file should contain exactly one &constants namelist'
      stop
    endif
    ...
    if(.NOT. (GYRE_DIR /= '')) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''GYRE_DIR /= '''' failed at line 122 <gyre_constants:read_constants>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'GYRE_DIR is not set'
      stop
    endif
  ...
  subroutine set_constant (name, value)
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'ABORT at line 151 <gyre_constants:set_constant>:'
    write(UNIT=ERROR_UNIT, FMT=*) 'Invalid name'
