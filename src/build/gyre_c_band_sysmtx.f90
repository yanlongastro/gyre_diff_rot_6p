...
!   uses: gyre_ext core_kinds gyre_sysmtx core_parallel core_linalg ISO_FORTRAN_ENV gyre_linalg
...
module gyre_c_band_sysmtx
  ...
  type, extends (c_sysmtx_t) :: c_band_sysmtx_t
     private
     complex(WP), allocatable        :: A_b(:,:) ! Banded matrix
  ...
  end type c_band_sysmtx_t

  ! Interfaces

  interface c_band_sysmtx_t
     module procedure c_band_sysmtx_t_
  end interface c_band_sysmtx_t

  ! Access specifiers

  private

  public :: c_band_sysmtx_t

  ! Procedures

contains

  function c_band_sysmtx_t_ (n, n_e, n_i, n_o) result (sm)

    integer, intent(in)      :: n
    integer, intent(in)      :: n_e
    integer, intent(in)      :: n_i
    integer, intent(in)      :: n_o
    type(c_band_sysmtx_t) :: sm
  ...
  end function c_band_sysmtx_t_

  !****

  subroutine set_B_i (this, B, scl)

    class(c_band_sysmtx_t), intent(inout) :: this
  ...
  if(SIZE(B, 1)/= this%n_i) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(B, 1)==this%n_i failed at line 20 <gyre_c_band_sysmtx:set_B_i>'
    stop
  endif
  ...
  if(SIZE(B, 2)/= this%n_e) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(B, 2)==this%n_e failed at line 20 <gyre_c_band_sysmtx:set_B_i>'
    stop
  endif
  ...
  if(SIZE(scl)/= this%n_i) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_i :', this%n_i
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(scl)==this%n_i failed at line 20 <gyre_c_band_sysmtx:set_B_i>'
    stop
  endif
  ...
  if(SIZE(B, 1)/= this%n_o) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(B, 1)==this%n_o failed at line 20 <gyre_c_band_sysmtx:set_B_o>'
    stop
  endif
  ...
  if(SIZE(B, 2)/= this%n_e) then
    write(UNIT=ERROR_UNIT, FMT=*) 'SIZE(B, 2) :', SIZE(B, 2)
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_e :', this%n_e
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(B, 2)==this%n_e failed at line 20 <gyre_c_band_sysmtx:set_B_o>'
    stop
  endif
  ...
  if(SIZE(scl)/= this%n_o) then
    write(UNIT=ERROR_UNIT, FMT=*) 'SIZE(scl) :', SIZE(scl)
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_o :', this%n_o
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(scl)==this%n_o failed at line 20 <gyre_c_band_sysmtx:set_B_o>'
    stop
  endif
  ...
  subroutine set_E (this, k, E_l, E_r, scl)
    ...
    integer   :: j_0
    integer   :: i
    complex(WP) :: R(2*this%n_e)

  if(SIZE(E_l, 1)/= this%n_e) then
    write(UNIT=ERROR_UNIT, FMT=*) 'SIZE(E_l, 1) :', SIZE(E_l, 1)
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_e :', this%n_e
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(E_l, 1)==this%n_e failed at line 20 <gyre_c_band_sysmtx:set_E>'
    stop
  endif
  ...
  if(SIZE(E_l, 2)/= this%n_e) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_e :', this%n_e
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(E_l, 2)==this%n_e failed at line 20 <gyre_c_band_sysmtx:set_E>'
    stop
  endif
  ...
  if(SIZE(E_r, 1)/= this%n_e) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_e :', this%n_e
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(E_r, 1)==this%n_e failed at line 20 <gyre_c_band_sysmtx:set_E>'
    stop
  endif
  ...
  if(SIZE(E_r, 2)/= this%n_e) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_e :', this%n_e
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(E_r, 2)==this%n_e failed at line 20 <gyre_c_band_sysmtx:set_E>'
    stop
  endif

    if(.NOT. (k >= 1)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''k >= 1'' failed at line 20 <gyre_c_band_sysmtx:set_E>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Invalid block index'
      stop
    endif

    if(.NOT. (k <= this%n)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''k <= this%n'' failed at line 20 <gyre_c_band_sysmtx:set_E>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Invalid block index'
      stop
    endif
  ...
  subroutine set_row_ (this, i_0, j_0, R)
    ...
    if(.NOT. (ABS(i_0-j_0) <= this%n_ul)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''ABS(i_0-j_0) <= this%n_ul'' failed at line 20 <gyre_c_band_sysmtx:set_row_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Invalid indices'
      stop
    endif

    if(.NOT. (ABS(i_0-(j_0+SIZE(R)-1)) <= this%n_ul)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''ABS(i_0-(j_0+SIZE(R)-1)) <= this%n_ul'' failed at line 20 <gyre_c_band_sysmtx:set' &
 & // '_row_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Invalid indices'
      stop
    endif
  ...
  subroutine get_row_ (this, i_0, j_0, R)
    ...
    class(c_band_sysmtx_t), intent(in) :: this
    integer, intent(in)                   :: i_0
    integer, intent(in)                   :: j_0
    ...
    if(.NOT. (ABS(i_0-j_0) <= this%n_ul)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''ABS(i_0-j_0) <= this%n_ul'' failed at line 20 <gyre_c_band_sysmtx:get_row_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Invalid indices'
      stop
    endif

    if(.NOT. (ABS(i_0-(j_0+SIZE(R)-1)) <= this%n_ul)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''ABS(i_0-(j_0+SIZE(R)-1)) <= this%n_ul'' failed at line 20 <gyre_c_band_sysmtx:get' &
 & // '_row_>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Invalid indices'
      stop
    endif
  ...
  subroutine factor (this)
    ...
    call ZGBTRF(m, m, this%n_ul, this%n_ul, this%A_b, n, this%ipiv, info)

    if(.NOT. (info == 0 .OR. info > m-this%n_e)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''info == 0 .OR. info > m-this%n_e'' failed at line 20 <gyre_c_band_sysmtx:factor>:'
  ...
  function det (this)
    ...
    det = product([c_ext_t(this%A_b(2*this%n_ul+1,:)),c_ext_t(this%scl_i),this%scl,c_ext_t(this%scl_o)])
  ...
  function soln_vec_hom (this) result (v)
    ...
    associate (n => this%n, n_e => this%n_e, n_ul => this%n_ul)
      ...
      call ZGBTRS('N', SIZE(A_b, 2), n_ul, n_ul, 1, A_b, SIZE(A_b, 1), ipiv, B, SIZE(B, 1), info)

    if(.NOT. (info == 0)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''info == 0'' failed at line 20 <gyre_c_band_sysmtx:soln_vec_hom>:'
  ...
  function soln_vec_inhom (this, w_i, w_o) result (v)
    ...
    complex(WP) :: B(this%n_e*(this%n+1),1)
    integer   :: info

  if(SIZE(w_i)/= this%n_i) then
    write(UNIT=ERROR_UNIT, FMT=*) 'SIZE(w_i) :', SIZE(w_i)
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_i :', this%n_i
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(w_i)==this%n_i failed at line 20 <gyre_c_band_sysmtx:soln_vec_inhom>'
    stop
  endif
  ...
  if(SIZE(w_o)/= this%n_o) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'this%n_o :', this%n_o
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(w_o)==this%n_o failed at line 20 <gyre_c_band_sysmtx:soln_vec_inhom>'
    stop
  endif
    ...
    associate (n => this%n, n_e => this%n_e, n_i => this%n_i, n_ul => this%n_ul)
      ...
      call ZGBTRS('N', n_e*(n+1), n_ul, n_ul, 1, &
                     this%A_b, SIZE(this%A_b, 1), this%ipiv, B, n_e*(n+1), info)

    if(.NOT. (info == 0)) then
      write(UNIT=ERROR_UNIT, FMT=*) 'ASSERT ''info == 0'' failed at line 20 <gyre_c_band_sysmtx:soln_vec_inhom>:'
      write(UNIT=ERROR_UNIT, FMT=*) 'Non-zero return from XGBTRS'
      stop
    endif
  ...
  function resd_vec (this, v) result (w)
    ...
    integer   :: j_a
    integer   :: j_b
    complex(WP) :: R(this%n_e*(this%n+1))
  ...
  if(SIZE(v)/= this%n_e*(this%n+1)) then
    ...
    write(UNIT=ERROR_UNIT, FMT=*) 'CHECK_BOUNDS SIZE(v)==this%n_e*(this%n+1) failed at line 20 <gyre_c_band_sysmtx:resd_vec>'
    stop
  endif
